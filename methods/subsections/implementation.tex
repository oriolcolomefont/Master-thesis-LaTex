...
Contrastive loss and triplet loss are two prominent loss functions employed in the fields of feature learning, similarity learning, and dimensionality reduction.
...
Triplet loss function directs the learning process by endeavoring to minimize the distance between the anchor and positive instances and maximize the distance between the anchor and negative instances. Contrastive loss, on the other hand, works on pairs of instances \cite{supercontrast}. The inclusion of a margin parameter in the loss function guarantees a minimum separation between positive and negative instances in the embedding space. Given the nature of our training needs, we propose to train by using the former.
