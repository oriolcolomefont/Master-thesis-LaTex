...
Our experiments can benefit from incorporating time-domain processing, which allows for more natural handling of temporal patterns and sequences in the data while avoiding windowing artifacts. While mel-spectrogram-based approaches have proven useful in various audio-related machine-learning tasks, their limitations lie in their representation of perceptual similarity. Spectrograms capture the distribution of frequencies over time, and small differences in spectrograms generally indicate high perceptual similarity from a human listener's perspective. However, there are cases where two audio signals can have significantly different spectrograms but still be perceived as musically similar. This discrepancy is due to the complexity of human auditory perception, which encompasses temporal patterns, phase relationships between frequencies, and higher-level musical structures. Therefore, relying solely on spectrogram distance to measure high-level music similarity may not consistently align with human perceptual judgments \cite{Kim2020OneStrategies}\cite{Mesostructures2023}.
...
As previously stated, the goal of minimizing this loss function is to learn discriminative embeddings, where similar examples are grouped closely together while dissimilar examples are placed farther apart in the embedding space.
